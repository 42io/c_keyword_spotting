...
#else
void kann_mt(kann_t *ann, int n_threads, int max_batch_size) { (void)ann; (void)n_threads; (void)max_batch_size; }
...
kann_t *kann_load_fp(FILE *fp)
{
	...
	fread(magic, 1, 4, fp);
	if (strncmp(magic, KANN_MAGIC, 4)) return 0;
